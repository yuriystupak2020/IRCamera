...
def enlarge_and_brighten_brightest_spot(frame, top_left, size, enlarge_factor=3, brightness_factor=1.5):
    ...
    (_, _, _, maxLoc) = cv2.minMaxLoc(gray_roi)
    ...
    enlarged_roi = frame[enlarged_top_left_y:enlarged_top_left_y + enlarged_size,
                   enlarged_top_left_x:enlarged_top_left_x + enlarged_size]

    hsv = cv2.cvtColor(enlarged_roi, cv2.COLOR_BGR2HSV)
    h, s, v = cv2.split(hsv)
    v = cv2.convertScaleAbs(v, alpha=brightness_factor)
    final_hsv = cv2.merge((h, s, v))
    frame[enlarged_top_left_y:enlarged_top_left_y + enlarged_size,
    enlarged_top_left_x:enlarged_top_left_x + enlarged_size] = cv2.cvtColor(final_hsv, cv2.COLOR_HSV2BGR)
...
# Function to decrease the brightness outside the red square
def decrease_brightness_outside_square(frame, top_left, size, brightness_factor=0.25):
    mask = np.ones(frame.shape, dtype="float32")
    if top_left is not None:
        bottom_right = (top_left[0] + size, top_left[1] + size)
        mask[top_left[1]:bottom_right[1], top_left[0]:bottom_right[0]] = 1.0  # keep the square unchanged
        mask[:top_left[1], :, :] = brightness_factor
        mask[bottom_right[1]:, :, :] = brightness_factor
        mask[:, :top_left[0], :] = brightness_factor
        mask[:, bottom_right[0]:, :] = brightness_factor

    # Convert frame to float32, perform multiplication, and convert back to uint8
    frame_float = frame.astype("float32")
    result = cv2.multiply(frame_float, mask)
...
while cap.isOpened():
    ...
    if red_square_top_left is not None:
        ...
        frame = decrease_brightness_outside_square(frame, red_square_top_left,
                                                   red_square_size)  # Decrease brightness outside the square
